...
export default async function handler(req,res){
 ...
 for(const [key,value] of Object.entries(req.query||{})){if(key==='path')continue;if(Array.isArray(value))value.forEach(v=>targetUrl.searchParams.append(key,String(v)));else if(value!=null)targetUrl.searchParams.append(key,String(value))}
 const headers=copyRequestHeaders(req);headers.set('x-tgs-proxy-secret',proxySecret);headers.set('x-forwarded-host',String(req.headers.host||''));headers.set('x-forwarded-proto','https')
 let body
 if(!['GET','HEAD'].includes(String(req.method||'GET').toUpperCase())&&req.body!==undefined&&req.body!==null){if(Buffer.isBuffer(req.body)||typeof req.body==='string')body=req.body;else body=JSON.stringify(req.body)}
 try{
