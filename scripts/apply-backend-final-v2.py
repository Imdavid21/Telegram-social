...
# Apply the remaining search/reaction/trust UI behavior to the latest mainline
# Home + Telegram summary policy without replacing those newer implementations.
...
state_insert = state_anchor + "  const [remoteSearch, setRemoteSearch] = useState<TelegramSearchResponse | null>(null)\n  const [remoteSearchLoading, setRemoteSearchLoading] = useState(false)\n  const [remoteSearchError, setRemoteSearchError] = useState('')\n"
if 'remoteSearchLoading' not in s:
    if state_anchor not in s: raise SystemExit('search state anchor missing')
...
if old_focus in s:
    ...
    raise SystemExit('search focus effect anchor missing')
...
old_search = '''    {searchOpen && <div className="sg-search-layer" role="dialog" aria-modal="true" aria-label="Search Supergram">
...
elif 'Search all Telegram history' not in s:
    raise SystemExit('search UI anchor missing')
p.write_text(s)
