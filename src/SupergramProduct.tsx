...
export default function SupergramProduct(){
  ...
  const discoveryObjects=useMemo(()=>keyConnected?objects:objects.filter(row=>Boolean(row.item.media)),[keyConnected,objects])
  ...
  useEffect(()=>{const handler=()=>{setKeyConnected(hasOpenAIKey());setRankingRevision(value=>value+1)};window.addEventListener('supergram:openai-key-changed',handler);return()=>window.removeEventListener('supergram:openai-key-changed',handler)},[])
  ...
  const visible=useMemo(()=>feed.filter(item=>{const source=channelMap.get(item.channelId);if(!source||hiddenPosts.has(item.id)||hiddenSources.has(source.id))return false;if(sourceFilter&&item.channelId!==sourceFilter)return false;if(homeMode==='groups'&&source.type!=='group')return false;if(homeMode==='for-you'&&!settings.includePrivateChatsInForYou&&source.type==='person')return false;if(homeMode==='for-you'&&!keyConnected&&!item.media)return false;if(item.outgoing&&(source.type==='person'||source.type==='group'))return false;return true}),[feed,channelMap,hiddenPosts,hiddenSources,sourceFilter,homeMode,settings.includePrivateChatsInForYou,keyConnected])
  ...
  return <div className="sg2-app">
    ...
    <main className="sg2-main"><header className="sg2-topbar"><a className="sg2-mobile-brand" href="/"><BrandMark/><strong>Supergram</strong></a><div className="sg2-top-actions"><button onClick={()=>openModal('search')} aria-label="Search"><SearchIcon/></button><button onClick={()=>openModal('sources')} aria-label="Telegram conversations"><MessageIcon/>{unread>0&&<i/>}</button></div></header>
      ...
      {destination==='home'&&<section className="sg2-home"><div className="sg2-home-tabs" role="tablist"><button className={homeMode==='for-you'?'is-active':''} onClick={()=>setHomeMode('for-you')}>For You</button><button className={homeMode==='following'?'is-active':''} onClick={()=>setHomeMode('following')}>Following</button><button className={homeMode==='groups'?'is-active':''} onClick={()=>setHomeMode('groups')}>Groups</button>{sourceFilter&&<button className="sg2-source-chip" onClick={()=>setSourceFilter(null)}>{channelMap.get(sourceFilter)?.title||'Source'} ×</button>}</div>
        <HomeBrief objects={discoveryObjects} onCatchUp={()=>{setHomeMode('groups');scrollTo({top:0,behavior:'smooth'})}} onTopic={openTopic}/>
    ...
    <aside className="sg2-context" aria-label="Live context"><div className="sg2-context-block"><span className="sg2-eyebrow">Right now</span><strong>{unread?`${unread} unread updates`:'Caught up'}</strong><p>{keyConnected?`${channels.filter(row=>Number(row.unread||0)>0).length} sources have unread activity.`:'Media-first mode until an OpenAI key is connected.'}</p></div><div className="sg2-context-block"><strong>Active sources</strong>{channels.filter(row=>Number(row.unread||0)>0).sort((a,b)=>Number(b.unread||0)-Number(a.unread||0)).slice(0,5).map(source=><button key={source.id} onClick={()=>openSource(source.id)}><span>{source.avatar?<img src={source.avatar} alt=""/>:source.initials}</span><em>{source.title}</em><b>{Number(source.unread||0)>99?'99+':source.unread}</b></button>)}</div><button className="sg2-context-search" onClick={()=>openModal('search')}>Search your world <SearchIcon/></button></aside>
