  ...
  return <div className="crm-app">
    ...
    <main className="crm-main">
      ...
      {view === 'contacts' ? <section className="crm-page crm-contacts-page">
        ...
        <div className="crm-table"><div className="crm-table-row crm-table-head"><span>Contact</span><span>Stage</span><span>Company</span><span>Next action</span><span>Last activity</span><span>Value</span></div>{filtered.map(row => <button className="crm-table-row" key={row.id} onClick={() => openContact(row.id)}><span className="crm-person"><span className="crm-avatar">{row.avatar ? <img src={row.avatar} alt="" /> : row.initials}</span><span><strong>{row.name}</strong><small>{row.username ? `@${row.username}` : 'Telegram'}</small></span></span><span>{stageLabel(row.meta.stage)}</span><span>{row.meta.company || 'Not set'}</span><span>{row.meta.nextAction || 'None'}</span><span>{formatActivity(row.lastActivity)}</span><span>{row.meta.value !== undefined ? formatMoney(row.meta.value) : 'Not set'}</span></button>)}</div>
      </section> : null}
