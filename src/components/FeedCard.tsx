...
import { BookmarkIcon, EyeIcon, HeartIcon, MessageIcon, MoreIcon, SearchIcon, SendIcon } from './Icons'
...
import { summarizeTelegramMessage, type SummaryContextMessage, type TelegramSummary } from '../lib/telegramSummary'
...
function SourceAvatar({channel}:{channel:Channel}){const[failed,setFailed]=useState(false);return <span className="sg2-post-avatar">{channel.avatar&&!failed?<img src={channel.avatar} alt="" loading="lazy" decoding="async" onError={()=>setFailed(true)}/>:avatarText(channel)}</span>}
function summaryCacheKey(item:FeedItem,text:string){return `supergram.ai.post-summary.v1:${item.id}:${item.edited?'e':'n'}:${text.length}`}
function readSummaryCache(key:string){try{const raw=sessionStorage.getItem(key);return raw?JSON.parse(raw) as TelegramSummary:null}catch{return null}}
function writeSummaryCache(key:string,value:TelegramSummary){try{sessionStorage.setItem(key,JSON.stringify(value))}catch{}}

export function FeedCard({item,channel,feedMode,favoriteSource,storyEntries=[],summarizePrivateChats,summaryContext=[],onSave,onRead,onFavoriteSource,onHideSource,onHidePost,onFeedback,onSourceOpen,onDiscussionOpen,index=0}:{item:FeedItem;channel:Channel;feedMode:FeedMode;favoriteSource:boolean;summarizePrivateChats:boolean;storyEntries?:StoryEntry[];summaryContext?:SummaryContextMessage[];onSave:(item:FeedItem)=>void;onRead:(item:FeedItem)=>void;onFavoriteSource:(channel:Channel)=>void;onHideSource:(channel:Channel)=>void;onHidePost:(item:FeedItem)=>void;onFeedback:(item:FeedItem,type:Extract<ViewerActionType,'more_like_this'|'less_like_this'>)=>void;onSourceOpen?:(channel:Channel)=>void;onDiscussionOpen?:(item:FeedItem)=>void;index?:number}){
 ...
 const canSummarize=hasOpenAIKey()&&Boolean(text.trim())&&(!privateSource||summarizePrivateChats)
 const cacheKey=summaryCacheKey(item,text)
 const contextKey=summaryContext.map(row=>row.messageId||row.timestamp||row.text.length).join(':')
 const[summary,setSummary]=useState<TelegramSummary|null>(()=>readSummaryCache(cacheKey))
 const[summaryBusy,setSummaryBusy]=useState(false)
 const[summaryError,setSummaryError]=useState('')
 const displayText=summary?.summary||summary?.headline||''
 const fallbackText=media?clip(text,220):clip(text,320)
 const shownText=displayText||fallbackText
 const isLong=shownText.length>420
 const visibleText=!expanded&&isLong?`${shownText.slice(0,420).trimEnd()}…`:shownText
 ...
 const totalReactions=reactions.reduce((sum,row)=>sum+Number(row.count||0),0)
 ...
 useEffect(()=>{const cached=readSummaryCache(cacheKey);summaryRequested.current=Boolean(cached);setSummary(cached);setSummaryBusy(false);setSummaryError('')},[cacheKey])
 useEffect(()=>{const el=root.current;if(!el||typeof IntersectionObserver==='undefined')return;const controller=new AbortController();const observer=new IntersectionObserver(entries=>{const visible=Boolean(entries[0]?.isIntersecting&&entries[0].intersectionRatio>=.55);if(visible){if(!impressed.current){impressed.current=true;recordViewerAction({type:'impression',itemId:item.id,channelId:item.channelId,timestamp:Date.now(),media:Boolean(media)})}if(dwell.current===null)dwell.current=Date.now();if(canSummarize&&!readSummaryCache(cacheKey)&&!summaryRequested.current){summaryRequested.current=true;setSummaryBusy(true);void summarizeTelegramMessage(text,{sourceType:channel.type||'channel',sourceName:channel.title,outgoing:item.outgoing,previousMessages:summaryContext},controller.signal).then(result=>{writeSummaryCache(cacheKey,result);setSummary(result)}).catch(err=>{if(!controller.signal.aborted)setSummaryError(String((err as Error)?.message||'AI summary unavailable.'))}).finally(()=>{if(!controller.signal.aborted)setSummaryBusy(false)})}}else if(dwell.current!==null){const seconds=(Date.now()-dwell.current)/1000;dwell.current=null;if(seconds>=1.5)recordViewerAction({type:'dwell',itemId:item.id,channelId:item.channelId,timestamp:Date.now(),value:seconds,media:Boolean(media)});else recordViewerAction({type:'skip',itemId:item.id,channelId:item.channelId,timestamp:Date.now(),value:seconds,media:Boolean(media)})}},{threshold:[.2,.55,.85]});observer.observe(el);return()=>{observer.disconnect();controller.abort()}},[item.id,item.channelId,media,canSummarize,text,channel.type,channel.title,item.outgoing,contextKey,cacheKey])
 ...
 return <article ref={root} className={`sg2-post ${media?'has-media':'is-text'} ${item.unread?'is-unread':''}`} data-feed-index={index}>
  <header className="sg2-post-head"><button className="sg2-post-source" type="button" onClick={()=>onSourceOpen?.(channel)}><SourceAvatar channel={channel}/><span><strong>{channel.title}</strong><small>{timeAgo(item.timestamp)}{item.edited?' · edited':''}{storySources>1?` · ${storySources} sources`:''}</small></span></button>{channel.scam&&<span className="sg2-warning">Scam</span>}{channel.fake&&<span className="sg2-warning">Fake</span>}<button className="sg2-post-more" type="button" onClick={()=>setMoreOpen(true)} aria-label="Post options"><MoreIcon/></button></header>
  {media&&<div className="sg2-post-media"><MediaRenderer media={media}/></div>}
  {(visibleText||summaryBusy)&&<div className="sg2-post-copy"><p>{summaryBusy&&!summary?'Summarizing…':visibleText}</p>{summary&&<small className="sg2-ai-inline">AI summary · source available</small>}{summaryError&&canSummarize?<small className="sg2-ai-inline">AI summary unavailable</small>:null}{isLong&&<button type="button" onClick={()=>setExpanded(value=>!value)}>{expanded?'Show less':'More'}</button>}</div>}
  {storySources>1&&<button className="sg2-provenance" type="button" onClick={()=>setSourcesOpen(true)}><span>{storySources} sources</span><small>View context</small></button>}
  <div className="sg2-post-actions"><button className={liked?'is-active':''} disabled={likeBusy} onClick={()=>void toggleLike()} aria-label="React"><HeartIcon/></button><button onClick={openDiscussion} aria-label="Reply"><MessageIcon/>{Number(item.comments||0)>0&&<b>{item.comments}</b>}</button><button disabled={item.noForwards} onClick={()=>void openShare()} aria-label="Forward"><SendIcon/></button><button className={item.saved?'is-active':''} onClick={save} aria-label="Save"><BookmarkIcon/></button></div>
  {(totalReactions>0||item.views)&&<div className="sg2-post-social"><span>{totalReactions>0?`${totalReactions} reaction${totalReactions===1?'':'s'}`:''}</span>{item.views&&<small>{item.views} views</small>}</div>}
