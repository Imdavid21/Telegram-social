...
function ProductPreview() {
  ...
  return <Paper variant="outlined" sx={{ overflow: 'hidden', bgcolor: '#0D100E', borderColor: 'divider', boxShadow: '0 36px 100px rgba(0,0,0,.35)' }}>
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '250px minmax(0,1fr) 260px' }, minHeight: 470 }}>
      <Box sx={{ borderRight: { md: 1 }, borderBottom: { xs: 1, md: 0 }, borderColor: 'divider', p: 2 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mt: .5, mb: 2 }}>Needs attention</Typography>
        <Stack gap={.8}>{rows.map(([name, copy, stage], index) => <Grow in timeout={250 + index * 120} key={name}>
          <Box sx={{ p: 1.25, borderRadius: 2, bgcolor: index === 0 ? 'rgba(208,255,79,.08)' : 'transparent', border: '1px solid', borderColor: index === 0 ? 'rgba(208,255,79,.18)' : 'transparent' }}>
            <Stack direction="row" justifyContent="space-between" gap={1}><Typography variant="body2" fontWeight={700}>{name}</Typography><Typography variant="caption" color={index === 0 ? 'primary.main' : 'text.secondary'}>{stage}</Typography></Stack>
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: .4 }}>{copy}</Typography>
          </Box>
        </Grow>)}</Stack>
      </Box>

      <Box sx={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}><Typography fontWeight={750}>Kadar</Typography><Typography variant="caption" color="text.secondary">@kadar</Typography></Box>
        <Stack sx={{ p: 2.5, flex: 1 }} gap={1.2}>
      ...
      <Box sx={{ borderLeft: { md: 1 }, borderTop: { xs: 1, md: 0 }, borderColor: 'divider', p: 2 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mt: .7 }}>CRM details stay attached to the Telegram peer.</Typography>
        <Divider sx={{ my: 2 }} />
        <Stack gap={1.6}>
          <Box><Typography variant="caption" color="text.secondary">Stage</Typography><Typography variant="body2" fontWeight={700}>Qualified</Typography></Box>
          <Box><Typography variant="caption" color="text.secondary">Next action</Typography><Typography variant="body2" fontWeight={700}>Offer two call slots</Typography></Box>
          <Box><Typography variant="caption" color="text.secondary">AI brief</Typography><Typography variant="body2">Strong sponsorship intent. No pricing objection yet.</Typography></Box>
        </Stack>
        <Box sx={{ mt: 3 }}>
          <LinearProgress variant="determinate" value={(step + 1) * 25} sx={{ height: 6, borderRadius: 99 }} />
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>{['Conversation loaded', 'Profile enriched', 'CRM context attached', 'Next action ready'][step]}</Typography>
        </Box>
      </Box>
    </Box>
  </Paper>
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
    <AppBar position="sticky" elevation={0} color="transparent" sx={{ backdropFilter: 'blur(16px)', borderBottom: 1, borderColor: 'divider', bgcolor: 'rgba(11,13,12,.82)' }}>
      <Toolbar sx={{ maxWidth: 1240, width: '100%', mx: 'auto', gap: 1.5 }}>
        ...
        <Typography fontWeight={800} sx={{ flex: 1 }}>Telegram CRM</Typography>
    ...
    <Container maxWidth="lg">
      <Box sx={{ py: { xs: 8, md: 13 }, display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1.05fr .95fr' }, gap: { xs: 5, md: 8 }, alignItems: 'center' }}>
        <Fade in timeout={450}>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>{backendReady ? 'Telegram connection is ready.' : 'Telegram connection is being checked.'}</Typography>
          </Box>
        </Fade>
        ...
        <Grow in timeout={700}>
          <Paper variant="outlined" sx={{ p: 2.5, bgcolor: 'background.paper' }}>
            <Stack gap={2.2}>
              ...
              ].map(([number, copy], index) => <Fade in timeout={500 + index * 180} key={number}><Stack direction="row" gap={1.5} alignItems="center"><Box sx={{ width: 34, height: 34, borderRadius: 2, bgcolor: 'rgba(208,255,79,.1)', color: 'primary.main', display: 'grid', placeItems: 'center', fontWeight: 800 }}>{number}</Box><Typography variant="body2">{copy}</Typography></Stack></Fade>)}
            </Stack>
          </Paper>
        </Grow>
      </Box>
      ...
      <Box id="product" sx={{ pb: { xs: 8, md: 13 } }}>
        ...
        <Typography color="text.secondary" sx={{ mb: 4, maxWidth: 760 }}>The inbox is where work starts. Pipeline, tasks, contacts, and AI are views over the same relationship context rather than disconnected modules.</Typography>
        <ProductPreview />
      </Box>
      ...
      <Box id="how-it-works" sx={{ py: { xs: 7, md: 11 }, borderTop: 1, borderColor: 'divider' }}>
        ...
        <Typography color="text.secondary" sx={{ mt: 1, mb: 4, maxWidth: 760 }}>The product removes the manual work between “we spoke on Telegram” and “the relationship is actually tracked.”</Typography>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3,1fr)' }, gap: 1.5 }}>
          {benefits.map((item, index) => <Grow in timeout={420 + index * 110} key={item.title}><Paper variant="outlined" sx={{ p: 2.5, minHeight: 190 }}><Box sx={{ color: 'primary.main', mb: 2 }}>{item.icon}</Box><Typography variant="h3">{item.title}</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>{item.copy}</Typography></Paper></Grow>)}
        </Box>
      </Box>
      ...
      <Box sx={{ py: { xs: 8, md: 12 }, borderTop: 1, borderColor: 'divider', textAlign: 'center' }}>
        ...
        <Typography color="text.secondary" sx={{ mt: 1.5, mb: 3, mx: 'auto', maxWidth: 700 }}>Connect the account you already use and turn conversations into a usable relationship system.</Typography>
    ...
    <Divider />
    <Container maxWidth="lg"><Stack direction={{ xs: 'column', sm: 'row' }} gap={2} justifyContent="space-between" sx={{ py: 3 }}><Typography variant="body2" color="text.secondary">Independent client using the Telegram API. Not affiliated with Telegram.</Typography><Stack direction="row" gap={2}><Button size="small" href="/privacy.html" color="inherit">Privacy</Button><Button size="small" href="/terms.html" color="inherit">Terms</Button></Stack></Stack></Container>
  </Box>
}
